...
(function(root){
  ...
  const evolvedSources={'evolved-knight':'./assets/evolved-knight.png','evolved-witch':'./assets/evolved-witch.png','evolved-alchemist':'./assets/evolved-alchemist.png','evolved-dragoon':'./assets/evolved-dragoon.png','evolved-ranger':'./assets/evolved-ranger.png','evolved-neutral':'./assets/evolved-neutral.png'};
  ...
  function get(id,hero='knight',branch){
    ...
    if(branch&&['power','technique'].includes(branch)){const index=groups[group].indexOf(id)*2+(branch==='technique'?1:0),row=Math.floor(index/5),bounds=root.BloomAtlas?.evolved?.[group]||[0,.125,.25,.375,.5,.625,.75,.875,1],cols=root.BloomAtlas?.evolvedColumns?.[group]||[0,.2,.4,.6,.8,1],col=index%5,left=cols[col]+.002,top=bounds[row]+.002,width=cols[col+1]-cols[col]-.004,height=bounds[row+1]-bounds[row]-.004;return {key:group+':'+id+':'+branch,hero:'evolved-'+group,src:evolvedSources['evolved-'+group],columns:5,rows:8,index,sizeX:100/width,sizeY:100/height,x:left/(1-width)*100,y:top/(1-height)*100};}
  ...
  const api={groups,sources,evolvedSources,get};
